...
useHead({
  ...
  meta: [
    { name: 'description', content: 'Erfassen Sie wichtige Kontakte und Kostenträger für Ihre Therapie-Praxis.' }
  ]
})
...
const saveAndContinue = async () => {
  ...
  try {
    ...
    const result = await store.saveContactsAndInsurance()
    
    if (result.success) {
      console.log('Kontakte & Kostenträger erfolgreich gespeichert:', result.data)
      
      // Zum nächsten Schritt navigieren
      store.nextStep()
      await navigateTo('/onboarding/schritt-5')
    } else {
      errorMessage.value = result.error || 'Fehler beim Speichern der Kontakte'
    }
  } catch (error) {
